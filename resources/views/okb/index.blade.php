<x-app-layout>
    ...
    <div class="py-6" x-data="{ kabupatenId: '{{ $selectedKabupatenId ?? '' }}' }">
        ...
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <h3 class="text-2xl font-bold">Data Organisasi Keagamaan Buddha</h3>
            
            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <!-- Filter Kabupaten -->
                <form method="GET" action="{{ route('okb.index') }}" class="flex gap-2">
                    <select 
                        name="kabupaten_id" 
                        x-model="kabupatenId"
        ...
        @if($selectedKabupatenId)
            <div class="mb-4 p-3 bg-blue-50 border-l-4 border-blue-500 text-blue-700 rounded">
                <p class="text-sm">
                    ...
                    Menampilkan OKB di <strong>{{ $kabupatens->find($selectedKabupatenId)->kabupaten ?? 'Kabupaten Terpilih' }}</strong>
                    <span class="ml-2 text-gray-600">({{ $okbs->total() }} data ditemukan)</span>
                </p>
            </div>
        @else
            <div class="mb-4 p-3 bg-gray-50 border-l-4 border-gray-400 text-gray-700 rounded">
                <p class="text-sm">
                    ...
                    Menampilkan OKB dari <strong>Semua Kabupaten</strong>
                    <span class="ml-2 text-gray-600">({{ $okbs->total() }} data ditemukan)</span>
                </p>
            </div>
        @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-200 dark:bg-zinc-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Nama OKB</th>
                            ...
                            @forelse($okbs as $index => $okb)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace">
                                        ...
                                        <div class="text-sm text-gray-500 dark:text-white">{{ $okb->no_registrasi ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
