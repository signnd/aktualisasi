        ...
        <div class="flex flex-col sm:flex-row w-full justify-between gap-4">
            <select wire:model.live="kabupaten_id"
                ...
                <input type="text" 
                   ...
                   placeholder="Cari nama SMB, alamat, IZOP, PPJG, NSSMB, Bapen..."
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        {{ $smb->status == 'Disetujui' ? 'bg-green-100 text-green-800' : 
                                           ($smb->status == 'Ditolak' ? 'bg-red-100 text-red-800' : 'bg-gray-50 text-gray-800') }}">
                                        {{ $smb->status ?? 'Pending' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $smb->status_verifikasi == 'approved' ? 'bg-green-100 text-green-800' : 
                                       ($smb->status_verifikasi == 'rejected' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
                                    {{ ucfirst($smb->status_verifikasi ?? 'pending') }}
                                </span>
                                @if($smb->status_verifikasi === 'pending')
                                <div class="flex justify-center space-x-2 mt-2">
                                    <form action="{{ route('smb.verify', $smb->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="status_verifikasi" value="approved">
                                        <button type="submit" class="text-white bg-green-500 hover:bg-green-600 px-2 py-1 text-xs rounded transition" onclick="return confirm('Setujui perubahan data ini?')">Setujui</button>
                                    </form>
                                    <form action="{{ route('smb.verify', $smb->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="status_verifikasi" value="rejected">
                                        <button type="submit" class="text-white bg-red-500 hover:bg-red-600 px-2 py-1 text-xs rounded transition" onclick="return confirm('Tolak perubahan data ini?')">Tolak</button>
                                    </form>
                                </div>
                                @endif
