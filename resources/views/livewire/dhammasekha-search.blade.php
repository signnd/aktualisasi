            ...
            <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-200 dark:bg-zinc-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Nama Dhammasekha</th>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Jenis</th>
                        ...
                        <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-500">
                            @forelse($dhammasekhas as $index => $d)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                        {{ $dhammasekhas->firstItem() + $index }}
                                    </td>
                                    <td class="px-6 py-4 whitespace">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $d->nama }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-300 ">IZOP: {{ $d->no_izop }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            {{ $d->status_verifikasi == 'TRUE' ? 'bg-green-100 text-green-800' : 
                                               ($d->status_verifikasi == 'FALSE' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800') }}">
                                            {{ $d->status_verifikasi ?? '' }}
                                        </span>
                                    </td>
                                    @endif
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('dhammasekha.show', $d) }}" 
                                            ...
                                            @if(auth()->user()->kabupaten_id === $d->kabupaten_id || auth()->user()->user_role === 'admin')
                                            <a href="{{ route('dhammasekha.edit', $d) }}" 
                                               class="text-green-600 hover:text-green-900 transition" title="Edit">
                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                                                </svg>
                                            </a>
                                            <form action="{{ route('dhammasekha.destroy', $d) }}" method="POST" 
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center">
                                        ...
                                        <div class="flex flex-col items-center justify-center text-gray-500">
                                            ...
                                            <p class="text-sm mt-1">Klik tombol "Tambah Dhammasekha" untuk menambahkan data baru</p>
                                        </div>
                                        @endif
                                    </td>
                                </tr>
