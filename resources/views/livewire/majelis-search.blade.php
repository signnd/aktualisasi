        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-200 dark:bg-zinc-700">
                        ...
                        <button wire:click="sortBy('nama_majelis')" class="flex items-center hover:text-blue-400 transition">
                        Nama Majelis
                        @if($sortField === 'nama_majelis')
                            @if($sortDirection === 'asc')
                                <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                    ...
                    <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-500">
                        @forelse($majeliss as $item)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                    {{ $loop->iteration + $majeliss->firstItem() - 1 }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $item->nama_majelis }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-200">Sekte: {{ $item->sekte ?? '-' }}</div>
                                    @if($item->binaan && $item->binaan !== '')
                                    <div class="text-xs text-gray-500 dark:text-gray-200">Binaan: {{ $item->binaan }}</div>
                                    @else
                                    <div class="text-xs text-gray-500 dark:text-gray-200">Binaan: -</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        @if(auth()->user()->kabupaten_id === $item->kabupaten_id || auth()->user()->user_role === 'admin')
