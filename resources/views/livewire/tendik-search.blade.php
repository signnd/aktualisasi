            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-200 dark:bg-zinc-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">Nama Lembaga</th>
                ...
                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-500">
                    @forelse($tendiks as $index => $t)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $tendiks->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace text-sm text-gray-900 dark:text-white">
                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $t->nama_tendik }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                @if($t->lembaga)
                                    <a href="{{ route(strtolower(class_basename($t->lembaga_type)) . '.show', $t->lembaga_id) }}" class="text-blue-600 hover:underline">
                                        {{ $t->nama_lembaga }}
                                    </a>
                                @else
                                    {{ $t->nama_lembaga }}
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    @if(auth()->user()->kabupaten_id === $t->kabupaten_id || auth()->user()->user_role === 'admin')
                                    <a href="{{ route('tendik.edit', $t) }}" 
                                       class="text-green-600 hover:text-green-900 transition" title="Edit">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('tendik.destroy', $t) }}" method="POST" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center">
                                ...
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <p class="text-lg font-medium">Belum ada data Tenaga Kependidikan</p>
