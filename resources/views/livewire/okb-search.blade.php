        ...
        <div class="flex flex-col sm:flex-row w-full justify-between gap-4">
            <select wire:model.live="kabupaten_id"
                ...
                <input type="text" 
                   ...
                   placeholder="Cari nama OKB, alamat, ketua, kabupaten..."
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-200 dark:bg-zinc-700">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-white uppercase tracking-wider">
                            <button wire:click="sortBy('id')" class="flex items-center hover:text-blue-400 transition">
                            No
                            @if($sortField === 'id')
                        ...
                        <button wire:click="sortBy('nama_okb')" class="flex items-center hover:text-blue-400 transition">
                        Nama OKB
                        @if($sortField === 'nama_okb')
                            @if($sortDirection === 'asc')
                                <svg class="w-4 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                ...
                <tbody class="bg-white dark:bg-zinc-800 divide-y divide-zinc-500">
                    @forelse($okbs as $index => $okb)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $okbs->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace">
                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $okb->nama_okb }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-300">{{ $okb->no_registrasi ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $okb->kabupaten->kabupaten ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $okb->kecamatan->kecamatan ?? '-' }}
                            </td>
                            @if(auth()->user()->user_role === 'admin')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $okb->status_verifikasi == 'approved' ? 'bg-green-100 text-green-800' : 
                                       ($okb->status_verifikasi == 'rejected' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
                                    {{ ucfirst($okb->status_verifikasi ?? 'pending') }}
                                </span>
                                @if($okb->status_verifikasi === 'pending')
                                <div class="flex justify-center space-x-2 mt-2">
                                    <form action="{{ route('okb.verify', $okb->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <form action="{{ route('okb.destroy', $okb) }}" method="POST" 
                        ...
                        <td colspan="6" class="text-center py-6 text-gray-500 dark:text-gray-400 italic">
                            ...
                            @else
                                <div class="flex flex-col items-center justify-center text-gray-500">
                                    ...
                                    <p class="text-sm mt-1">Klik tombol "Tambah OKB" untuk menambahkan data baru</p>
                                </div>
                            @endif
                        </td>
                    </tr>
